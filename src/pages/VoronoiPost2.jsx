...
const interopCells = {
  ...
  adlsice: { spark:"native",    trino:"native",    flink:"native",    duckdb:"bridge",    snowflake:"bridge",    bigquery:"bridge",    databricks:"native",    fabric:"native",    athena:"bridge",    onprk:"bridge"    },
...
const interopMechanisms = {
  s3ice: {
    spark:      { type:"native",    detail: "Polaris REST Catalog resolves table path. Spark reads Parquet via S3A driver. No vendor SDK — pure Iceberg spec." },
    trino:      { type:"native",    detail: "Trino Iceberg connector + Polaris REST Catalog. Vectorised reads via S3A. Production-grade, no cluster warm-up." },
    flink:      { type:"native",    detail: "Flink Iceberg sink writes; Flink source reads. Full streaming ACID support on S3. Same Polaris REST Catalog." },
    duckdb:     { type:"native",    detail: "DuckDB httpfs or AWS S3 extension reads Iceberg metadata + Parquet files directly. Read-only; zero infrastructure." },
    snowflake:  { type:"bridge",    detail: "Snowflake External Volume on S3. Iceberg Tables feature (GA). Polaris REST Catalog optional; Snowflake can manage metadata. Needs AWS credential in Snowflake storage integration." },
    bigquery:   { type:"bridge",    detail: "BigLake external table with connection to S3. Cross-cloud — BigQuery queries Parquet on S3. Requires GCP–AWS credential link. Preview feature; egress cost applies." },
    databricks: { type:"native",    detail: "Databricks reads Iceberg natively on S3. Can use Polaris REST Catalog or Databricks-managed catalog. No format conversion." },
    fabric:     { type:"bridge",    detail: "Fabric OneLake Shortcut to S3 Iceberg table. Reads Parquet files via shortcut. Power BI and Fabric notebooks consume without copy. Preview; credential delegation required." },
    athena:     { type:"native",    detail: "Native Iceberg support on S3 (Athena v3). Uses Glue Catalog or Iceberg REST Catalog. Optimistic concurrency for writes. Zero cluster management." },
    onprk:      { type:"bridge",    detail: "On-prem Spark reads S3 via S3A driver with AWS credential (IAM role or access key). Polaris REST Catalog reachable over VPN/PrivateLink. Standard Iceberg after credential setup." },
  },
  adlsice: {
    spark:      { type:"native",    detail: "ABFS driver + Polaris REST Catalog. Entra service principal or managed identity for auth. Same Iceberg spec — storage driver is the only Azure-specific dependency." },
    trino:      { type:"native",    detail: "Trino Azure connector (ABFS) + Polaris REST. Entra credential in Trino config. Full Iceberg read/write once auth is wired." },
    flink:      { type:"native",    detail: "Flink ADLS Gen2 connector + Iceberg sink. Streaming writes to Iceberg on ADLS. Entra service principal auth." },
    duckdb:     { type:"bridge",    detail: "DuckDB Azure extension (azure_storage_scan). More config than S3 — requires Azure connection string or SAS token. Read-only." },
    snowflake:  { type:"bridge",    detail: "Snowflake External Volume on ADLS Gen2. Iceberg Tables feature. Needs Azure storage integration in Snowflake with Entra credential." },
    bigquery:   { type:"bridge",    detail: "BigLake connection to ADLS Gen2 (cross-cloud). Requires GCP–Azure credential bridge. Less mature than GCS BigLake. Egress applies." },
    databricks: { type:"native",    detail: "Databricks on Azure natively reads ADLS Gen2. Delta and Iceberg both supported. Managed identity or service principal auth built into cluster config." },
    fabric:     { type:"native",    detail: "ADLS Gen2 is OneLake's underlying storage plane. Fabric reads Iceberg tables on ADLS without a Shortcut — it is the home location. Full read/write." },
    athena:     { type:"bridge",    detail: "Athena is AWS-native. Querying ADLS requires S3-compatible proxy or data movement. Not a natural path — on-prem or replication is more practical." },
    onprk:      { type:"bridge",    detail: "On-prem Spark with ABFS driver reads ADLS Gen2. Entra service principal + network path (ExpressRoute / VPN) required. Polaris REST Catalog accessible over same network path." },
  },
  gcsice: {
    spark:      { type:"native",    detail: "GCS connector + Polaris REST Catalog. Service account key or Workload Identity for auth. Standard Iceberg reads after credential config." },
    trino:      { type:"native",    detail: "Trino GCS connector + Polaris REST. Service account auth. Vectorised Parquet reads. Same Iceberg spec as S3/ADLS." },
    flink:      { type:"native",    detail: "Flink GCS connector + Iceberg sink/source. Dataflow on GCP runs managed Flink jobs writing Iceberg on GCS." },
    duckdb:     { type:"bridge",    detail: "DuckDB GCS extension (experimental). Requires GCS credentials. Functional but less battle-tested than S3 path." },
    snowflake:  { type:"bridge",    detail: "Snowflake External Volume on GCS. Iceberg Tables feature. GCS storage integration with service account credential." },
    bigquery:   { type:"native",    detail: "BigLake — BigQuery's native path for Iceberg on GCS. Full BigQuery engine on open Iceberg format. No proprietary storage, BigQuery compute only." },
    databricks: { type:"native",    detail: "Databricks on GCP natively reads GCS. Iceberg tables supported. Service account auth. Delta and Iceberg both available." },
    ...
    onprk:      { type:"bridge",    detail: "On-prem Spark with GCS connector + service account JSON key. Network path to GCP required (VPN / Dedicated Interconnect). Iceberg reads work after credential and network setup." },
  },
  onprem: {
    spark:      { type:"native",    detail: "catalog-gateway (Iceberg REST, JWT bearer) → OPA (4 layers: allow/deny, virtual-manifest RLS, column mask advisory, schema exclusion) → Nessie (PostgreSQL-backed, never host-exposed) → MinIO S3A. Cloud-sourced tables need a full metadata URI rewrite — Iceberg embeds absolute paths, so every pointer must resolve to MinIO before on-prem Spark can read them. → github.com/techbitsvsk/catalog_sync" },
    trino:      { type:"native",    detail: "Same catalog-gateway as Spark. L2 RLS is transparent: gateway writes a filtered manifest-list to MinIO and swaps the snapshot URL in the GET response — Trino downloads only permitted partition files with no cooperation required. → github.com/techbitsvsk/catalog_sync" },
    flink:      { type:"native",    detail: "Flink Iceberg sink commits streaming snapshots via catalog-gateway → Nessie. OPA validates write permission (namespace classification + table policy) before the register call forwards. Full streaming ACID on-prem." },
    duckdb:     { type:"bridge",    detail: "DuckDB httpfs reads Parquet from MinIO S3-compatible endpoint. Iceberg metadata resolved via catalog-gateway REST (JWT required). Read-only — no DuckDB write path to Nessie. MinIO must be network-reachable." },
    snowflake:  { type:"bridge",    detail: "Snowflake External Volume on MinIO S3-compatible API over PrivateLink or VPN. catalog-gateway REST must be reachable from Snowflake's network — Snowflake resolves Iceberg metadata via REST, reads Parquet directly from MinIO." },
    ...
    databricks: { type:"bridge",    detail: "Databricks (cloud) reaches on-prem MinIO over ExpressRoute/DirectConnect. catalog-gateway is registered as the external catalog REST endpoint. BYOC or partner cluster configs support regulated environments." },
    ...
    athena:     { type:"bridge",    detail: "Athena treats MinIO as S3 via S3-compatible API over VPN/PrivateLink. catalog-gateway REST must be reachable from the Athena VPC. Functional but not AWS first-class — network path required." },
    onprk:      { type:"native",    detail: "Native. catalog-gateway + Nessie + MinIO. OAuth2 JWT for auth; OPA 4-layer enforcement (L1 allow/deny, L2 virtual-manifest RLS, L3 column mask advisory, L4 schema exclusion). Nessie never host-exposed — gateway mediates all REST calls. → github.com/techbitsvsk/catalog_sync" },
  },
  snow: {
    spark:      { type:"connector", detail: "Snowflake Spark Connector (proprietary JDBC/Arrow protocol). Functional for reads and writes. Creates a hard dependency on Snowflake SDK JAR versions. Not open spec." },
    trino:      { type:"connector", detail: "Snowflake JDBC connector for Trino. Read-only via standard SQL over JDBC. Proprietary protocol — not Iceberg REST. Performance limited by JDBC throughput." },
    ...
    duckdb:     { type:"connector", detail: "DuckDB Snowflake scanner extension (community). Pushes SQL to Snowflake, returns Arrow batches. Convenient for analyst workflows. Proprietary Snowflake protocol." },
  ...
  onelake: {
    ...
    trino:      { type:"bridge",    detail: "Trino Iceberg connector + OneLake Iceberg REST Catalog endpoint. Entra service principal credential in Trino catalog config. REST spec resolves table → metadata path. Vectorised Parquet reads via OneLake. Bridge: Entra credential config required, not zero-config, but no proprietary SDK." },
    flink:      { type:"bridge",    detail: "Flink Iceberg source + OneLake REST Catalog endpoint (https://onelake.table.fabric.microsoft.com/iceberg). Entra auth token in Flink config. Streaming reads from OneLake Iceberg tables via open Iceberg REST spec. Bridge: credential setup required, otherwise standard Iceberg protocol." },
    duckdb:     { type:"bridge",    detail: "Confirmed: DuckDB connects to OneLake via the Iceberg REST Catalog endpoint as an Iceberg-compatible client. OneLake Iceberg REST docs explicitly list DuckDB as a supported client. DuckDB resolves table metadata via REST, reads Parquet files. Read-only. Bridge: Entra credential and OneLake workspace URL required." },
    snowflake:  { type:"bridge",    detail: "GA. Snowflake↔OneLake interoperability is generally available. Snowflake creates a catalog integration pointing to the OneLake Iceberg REST endpoint. Snowflake queries OneLake Iceberg tables directly without ETL or data movement. Bidirectional: Snowflake-managed Iceberg tables can also be stored natively in OneLake. Entra auth via Snowflake storage integration. Documented at docs.snowflake.com/catalog-integration-rest-onelake." },
    ...
    databricks: { type:"bridge",    detail: "Databricks connects to OneLake via the Iceberg REST Catalog endpoint with Entra service principal. Iceberg tables in OneLake are discoverable as external tables from Databricks. Delta Sharing from Fabric to Databricks (open Delta Sharing protocol) is an alternative path. Bridge: Entra credential required, but open spec." },
    ...
    onprk:      { type:"bridge",    detail: "On-prem Spark connects to OneLake Iceberg REST endpoint over ExpressRoute / VPN with an Entra service principal credential. Same Iceberg REST spec as cloud Spark — the network path and credential are the only differences. Bridge: network architecture and Entra credential required." },
  },
};
            ...
            <div key={src.id} style={{ display: "grid", gridTemplateColumns: `160px repeat(${consumers.length}, ${colW}px)`, borderBottom: ri < sources.length - 1 ? "1px solid #141414" : "none" }}>
              ...
              {consumers.map(con => {
                ...
                return (
                  ...
                  >
                    {t === "self"
                      ? <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.6rem", color: "#2a2a2a" }}>—</span>
                      : t === "none"
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "3rem 2rem 6rem" }}>
        ...
        <section style={{ marginBottom: "3rem" }}>
          ...
          <p style={{ ...prose, color: "#6a5a4a" }}>
            The practical pattern: AWS for primary Iceberg storage (S3 cost depth), Azure for identity
            and governance (Entra ID, Purview, Fabric reporting surface), GCP for AI/ML training and
            streaming ingestion (Vertex, Dataflow). The Polaris REST Catalog endpoint sits in front of
            all three — a single table registry, every engine, every cloud.
          </p>
        </section>
